...
class HALNode(Node):
    def __init__(self, default_iface='can2', name='hal_node_jetson'):
        super().__init__(name)
    ...
    def _on_joy(self, msg: Joy):
        ...
        self.joy_throttle = max(0.0, msg.axes[1]) * 100.0   # throttle %
        self.joy_brake    = max(0.0, -msg.axes[1]) * 100.0  # brake %
        self.joy_steer    = msg.axes[3] * 30.0              # steering -30..+30°
    ...
    def _tick(self):
        ...
        dbg = f"[HAL Jetson] th={throttle:.1f}% br={brake:.1f}% st={steer:.1f}°"
        if self.last_tsd is not None:
...
def main(argv=None):
    ...
    node = HALNode(default_iface='can2', name='hal_node_jetson')
    try:
        rclpy.spin(node)
